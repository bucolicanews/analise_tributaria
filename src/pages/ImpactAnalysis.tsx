...
const ImpactAnalysis = () => {
  ...
  const analysisData = useMemo(() => {
    ...
    try {
      ...
      const totalFutureDebits = calculatedFuture.reduce((sum, p) => sum + (p.cbsDebit + p.ibsDebit + p.selectiveTaxToPay + p.simplesToPay) * p.quantity, 0);
  ...
  const isSaldoCredor = futureResult.totalTax < 0;
    ...
    <div className="container mx-auto px-4 py-8 space-y-8">
            ...
            <div className="p-4 border border-primary/20 rounded-lg bg-primary/5">
               ...
               <div className="mt-4 pt-4 border-t border-border/50">
                 <div className="flex justify-between items-center cursor-pointer" onClick={() => setShowTaxDetail(!showTaxDetail)}>
                   <span className="text-[10px] font-bold text-muted-foreground uppercase">Impostos (Saldo Líquido)</span>
                 ...
                 <CalculationStep 
                   label={isSaldoCredor ? "Saldo Credor Acumulado" : "Imposto Líquido a Pagar"} 
                   value={futureResult.totalTax} 
                   isNegative={isSaldoCredor}
                   color={isSaldoCredor ? "text-success" : "text-destructive"} 
                 />
               </div>
      ...
      <div className="grid md:grid-cols-3 gap-6">
        ...
        <Card className="bg-success/10 border-success/30 p-6 text-center flex flex-col justify-center">
          ...
          <h3 className="text-[10px] font-bold text-success uppercase">Ganho na Reforma</h3>
