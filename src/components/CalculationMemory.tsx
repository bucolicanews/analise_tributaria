...
export const CalculationMemory = ({ products, params }: CalculationMemoryProps) => {
  ...
  if (params.taxRegime === TaxRegime.LucroPresumido) {
    ...
    if (params.generateIvaCredit) { // Simples Nacional Híbrido (gera crédito de IVA)
      totalPercentageForMarkup =
        (totalVariableExpensesPercentage + params.simplesNacionalRate + params.profitMargin + cbsRateEffective + ibsRateEffective + selectiveTaxRateEffective) / 100;
    } else { // Simples Nacional Padrão (não gera crédito de IVA)
      totalPercentageForMarkup =
        (totalVariableExpensesPercentage + params.simplesNacionalRate + params.profitMargin + selectiveTaxRateEffective) / 100;
    }
  }
            ...
            {params.taxRegime === TaxRegime.LucroPresumido ? (
              ...
              <>
                ...
                {params.generateIvaCredit && (
                  <>
                    <br/><strong>Alíquotas IVA Efetivas:</strong> CBS ({formatPercent(cbsRateEffective)}), IBS ({formatPercent(ibsRateEffective)})
                  </>
                )}
      ...
      {markupDivisor <= 0 || baseCostForMarkupExample === Infinity ? (
        ...
        <div className="space-y-4 rounded-lg bg-muted/30 p-6 font-mono text-sm">
          <div>
            ...
            <p className="ml-4">
              • Crédito PIS/COFINS (CBS Crédito) por Unid. Com.: {params.usePisCofins ? `${formatCurrency(firstProduct.pisCredit)} + ${formatCurrency(firstProduct.cofinsCredit)} = ${formatCurrency(calculated.cbsCredit)}` : "Inativo (0,00)"}
            </p>
            <p className="ml-4">
              • Crédito ICMS (IBS Crédito) por Unid. Com.: {formatCurrency(firstProduct.icmsCredit)} * {formatPercent(params.icmsPercentage)} = {formatCurrency(calculated.ibsCredit)}
            </p>
          ...
          <div>
            ...
            <p className="ml-4">
              {formatCurrency(baseCostForMarkupExample)} ÷ (1 - ({formatPercent(totalVariableExpensesPercentage)} + {params.taxRegime === TaxRegime.SimplesNacional ? (params.generateIvaCredit ? `${formatPercent(params.simplesNacionalRate)} + ${formatPercent(cbsRateEffective)} + ${formatPercent(ibsRateEffective)}` : formatPercent(params.simplesNacionalRate)) : `${formatPercent(cbsRateEffective)} + ${formatPercent(ibsRateEffective)}`} + {formatPercent(selectiveTaxRateEffective)}))
            </p>
          ...
          <div>
            ...
            {params.taxRegime === TaxRegime.LucroPresumido ? (
              <>
                ...
                <p className="ml-4">
                  • IBS a Pagar por Unid. Com.: Débito IBS ({formatCurrency(calculated.ibsDebit)}) - Crédito IBS ({formatCurrency(calculated.ibsCredit)}) = {formatCurrency(calculated.ibsTaxToPay)}
                </p>
